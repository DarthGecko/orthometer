...
def replace_headers(fasta,attributeset):
    ...
    def format_headers(matchobj):
        vs = matchobj.group(0).replace('>', '').split('|')
        #print vs
        out_string = '>'
        #print attrSelections[attributeset][1]
...
def export_results(result, outname, attributeset):
    o = open(outname, 'w')
    o.write(result)
    o = open(outname+'_header_order', 'w')
    datatypes = '\t'.join(attrSelections[attributeset][0])
    positions = str(attrSelections[attributeset][1])
    result_meta = 'data in header ordered as follows\n{}\n{}'.format(datatypes, positions)
    o.write(result_meta)
...
if __name__ == "__main__":
    ...
    good_keggs =''
    if (fts == 'ribtest'):
        print "Checking ribosomal kegg ids!\n"
        for kegg_id in filtSelections['ribosome'][1].split(','):
            try:
                filtSelections['ribtest'][1] = kegg_id
            except IndexError:
                filtSelections['ribtest'].append(kegg_id)
            myQuery = make_my_xml(fts, ats)
            resulting_data = get_results(myQuery)
            if len(resulting_data) == 0:
                print 'Error on kegg id {}\n'.format(kegg_id)
            else:
                good_keggs = "{}{}, ".format(good_keggs,kegg_id)
        print "The kegg ids for ribosomal protein that worked are \n{}".format(good_keggs)
    else:
